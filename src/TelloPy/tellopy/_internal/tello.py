...
class Tello(object):
    ...
    def __init__(self, port, ifname, tello_ns):
        ...
        self.log = logger.Logger(tello_ns)
        self.exposure = 0
    ...
    def __process_packet(self, data):
        ...
        if cmd == LOG_HEADER_MSG:
            ...
            self.log.info("recv: altitude limit: %s" % byte_to_hexstring(data[9:-2]))
        elif cmd == ATT_LIMIT_MSG:
            self.log.info("recv: attitude limit: %s" % byte_to_hexstring(data[9:-2]))
        elif cmd == LOW_BAT_THRESHOLD_MSG:
            self.log.info("recv: low battery threshold: %s" % byte_to_hexstring(data[9:-2]))
        elif cmd == LIGHT_MSG:
            self.log.debug("recv: light: %s" % byte_to_hexstring(data[9:-2]))
            ...
            self.log.debug("recv: ack: cmd=0x%02x seq=0x%04x %s" %
    ...
    def __video_thread(self):
        ...
        port = 6038
        sock.setsockopt(socket.SOL_SOCKET, 25, self.ifname.encode())
        sock.bind(('', port))
        sock.settimeout(1.0)
